      ...
      (let [msg (first @messages)]
       ...
       (when msg
          (condp = (:type msg)
            ...
            c/heart-beat-response nil
            c/game-link-message (print-game-linke-message msg)
            c/tournament-ended-message (println "Tournament ended")
            c/game-result-message (println "Game result message")
            c/no-active-tournament-message (println "No active tournament")
            c/invalid-message (println "Invalid message: " msg)
            "user" (println (:content msg)))))
      (when (or (s/state-get :socket-open)
              (> (count @messages) 0))
         (recur))))
